<template>
    <div>
        ...
        <EditPost v-if="editingPost" :title="editingPost.title" :content="editingPost.content" :ingredients="editingPost.ingredients" :steps="editingPost.steps" :index="editingPostIndex" @updatePost="updatePost" @closeEdit="closeEdit" />
        <DeletePost v-if="deletingPost" :title="deletingPost.title" @deletePost="deletePost" @closeDelete="closeDelete" />
    </div>
  </template>
  
  <script setup>
  import { ref } from 'vue';
  import CreateRecipe from './Action__Posts/CreateRecipe.vue'; 
